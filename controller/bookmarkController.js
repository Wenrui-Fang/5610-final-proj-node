...
const findAllBookmarkByUser = async(req, res) => {
    ...
    const bookmarkedBusinessIds = bookmarks.map(bookmark => bookmark.businessId);
...
const userDeleteBookmarkedBusiness = async(req, res) => {
    const status = await bookmarkDao.userDeleteBookmarkedBusiness(req.params.uid, req.params.bid);
    res.json(status);
}

const userBookmarkedBusinessOrNot = async(req, res) => {
    const response = await bookmarkDao.userBookmarkedBusinessOrNot(req.params.uid, req.params.bid);
