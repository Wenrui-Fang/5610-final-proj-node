...
const userFollowsUser = async(req, res) => {
    const follow = await followDao.userFollowsUser(req.params.uid1, req.params.uid2);
    res.json(follow);
}

const userUnfollowUser = async(req, res) => {
    const status = await followDao.userUnfollowUser(req.params.uid1, req.params.uid2);
    res.json(status);
}
